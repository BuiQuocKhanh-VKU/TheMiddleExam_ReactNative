...
export default function AdminScreen() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    const filtered = kw
      ? users.filter((u) => (u.username || "").toLowerCase().includes(kw))
      : users;
      ...
      <Modal visible={!!editing} transparent animationType="slide" onRequestClose={() => setEditing(null)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            />
            <TextInput style={styles.input} placeholder="Password (Firestore)" value={uPass} onChangeText={setUPass} />

            <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
              <TouchableOpacity style={[styles.btn, { flex: 1, backgroundColor: "#ddd" }]} onPress={() => setEditing(null)}>
                <Text>Hủy</Text>
              </TouchableOpacity>
              <TouchableOpacity
            ...
            <Text style={styles.note}>
              ⚠️ Khuyến nghị: không lưu mật khẩu dưới dạng plaintext trong Firestore. Dùng Firebase Auth để quản lý
              mật khẩu và reset khi cần.
            </Text>
          </View>
        </View>
      </Modal>
