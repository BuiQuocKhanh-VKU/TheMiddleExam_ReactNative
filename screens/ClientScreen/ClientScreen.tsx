...
export default function ClientScreen() {
   ...
   const onAsk = async () => {
      ...
      try {
         setLoading(true);
         setAns("Đang xử lý...");
         const reply = await askGemini(q.trim());
         setAns(reply);
      } catch (e: any) {
         const msg = e?.message || "Lỗi gọi Gemini";
         console.log("Gemini error >>>", msg);
         setAns(msg);
         Alert.alert("AI lỗi", msg);
      } finally {
         setLoading(false);
      }
   };
            ...
            <View style={{ padding: 16 }}>
               ...
               >
                  {loading ? (
                     ...
                     <Text style={{ color: "#fff", fontWeight: "700" }}>Hỏi Gemini</Text>
                  )}
               </TouchableOpacity>
               ...
               {!!ans && (
                  ...
                  >
                     <Text style={{ fontWeight: "700", marginBottom: 6 }}>Trả lời:</Text>
                     <Text>{ans}</Text>
                  </View>
               )}
            </View>
